<template>
  <div class="min-h-screen flex justify-center items-start sm:py-10" :style="{ backgroundColor: pageTheme.background }">
    <!-- Main Container -->
    <main class="w-full sm:max-w-[480px] bg-noise min-h-screen sm:min-h-0 sm:rounded-[40px] shadow-2xl relative overflow-hidden flex flex-col items-center pb-8">
      ...
      <template v-else-if="data">
        ...
        <header class="w-full flex justify-between items-center px-5 py-4">
          <button @click="showMenu = !showMenu" class="w-10 h-10 flex items-center justify-center rounded-full hover:bg-black/5 transition-colors">
        ...
        <div class="flex flex-col items-center mt-2 mb-6 px-4">
          <div class="relative">
            <div class="w-[96px] h-[96px] rounded-full bg-gray-200 overflow-hidden flex items-center justify-center">
              <img v-if="data.profile?.avatar_url" :src="data.profile.avatar_url" :alt="data.username" class="w-full h-full object-cover">
              <div v-else class="w-full h-full flex items-center justify-center bg-black text-white font-bold text-2xl">
                {{ data.username?.charAt(0)?.toUpperCase() || '?' }}
              </div>
            </div>
          </div>
          <h1 class="text-[22px] font-bold text-black mt-4 tracking-tight">{{ data.profile?.full_name || '@' + data.username }}</h1>
          <p v-if="data.profile?.bio" class="text-sm font-medium text-gray-500 text-center mt-1 max-w-[250px]">{{ data.profile.bio }}</p>
        </div>
        ...
        <!-- Footer -->
        <div class="mt-auto w-full flex flex-col items-center pb-4 px-4 mt-10">
...
const pageTheme = computed(() => ({
  background: data.value?.theme?.background_color || '#B4B4B4',
}))
...
const shareData = computed(() => ({
  title: `${data.value?.username || 'Linkto'} on Linkto`,
  text: `Check out ${data.value?.username || 'this profile'} on Linkto!`,
